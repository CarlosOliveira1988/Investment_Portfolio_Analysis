...
def calcTotalProfitValues(initial_value, monthly_interest_rate_list):
    ...
    """
    total_profit_value = __calcTotalProfitValueFromMonthlyInterestRateList(
        initial_value, monthly_interest_rate_list)
    total_interest_rate = __calcTotalInterestRate(
        initial_value, total_profit_value)
...
def printTotalProfitValues(initial_value, total_profit_value, total_interest_rate, indexer_name, initial_date, final_date):
    ...
    print("Juros Total: R$ {:.2f}".format(
        initial_value * total_interest_rate/100))
...
def convertMonthlyInterestRateListToTable(monthly_interest_rate_list):
    ...
    """
    monthly_date_list = list(pd.date_range(
        start=SOURCE_FILE_INITIAL_DATE, periods=len(monthly_interest_rate_list), freq="BMS"))
    monthly_interest_rate_dict = {
        "Data Base": monthly_date_list, "Taxa Mensal": monthly_interest_rate_list}
    monthly_interest_rate_table = pd.DataFrame(
        monthly_interest_rate_dict, columns=["Data Base", "Taxa Mensal"])
...
def cutMonthlyInterestRateTable(monthly_interest_rate_table, initial_date, final_date):
    ...
    monthly_df_filtered = monthly_df.loc[(monthly_df["Data Base"] >= initial_date) & (
        monthly_df["Data Base"] <= final_date)]
    return monthly_df_filtered


def convertMonthlyInterestRateTableToList(monthly_interest_rate_table):
    monthly_interest_rate_table_short = monthly_interest_rate_table.drop([
                                                                         "Data Base"], axis=1)
    monthly_interest_rate_table_short = monthly_interest_rate_table_short.stack(
        dropna=True)
...
def calcTotalProfitValuesFromDate(indexer_name, initial_value, initial_date, final_date):
    monthly_interest_rate_list = convertExcelToMonthlyInterestRateList(
        indexer_dict[indexer_name])
    monthly_interest_rate_table = convertMonthlyInterestRateListToTable(
        monthly_interest_rate_list)
    monthly_interest_rate_short_table = cutMonthlyInterestRateTable(
        monthly_interest_rate_table, initial_date, final_date)
    monthly_interest_rate_short_list = convertMonthlyInterestRateTableToList(
        monthly_interest_rate_short_table)
    total_profit_value, total_interest_rate = calcTotalProfitValues(
        initial_value, monthly_interest_rate_short_list)
    return total_profit_value, total_interest_rate


def calcPrintTotalProfitValuesFromDate(indexer_name, initial_value, initial_date, final_date):
    profit_value, interest_rate = calcTotalProfitValuesFromDate(
        indexer_name, initial_value, initial_date, final_date)
    printTotalProfitValues(initial_value, profit_value,
                           interest_rate, indexer_name, initial_date, final_date)
...
def teste(initial_value, monthly_interest_rate_list):
    ...
    for i in range(len(monthly_interest_rate_list)):
        adjusted_monthly_interest_rate = (
            1 + monthly_interest_rate_list.iloc[i, 1]/100)
        total_profit_value = adjusted_monthly_interest_rate * total_profit_value
...
monthly_interest_rate_table = convertMonthlyInterestRateListToTable(
    monthly_interest_rate_list)
monthly_interest_rate_table = cutMonthlyInterestRateTable(
    monthly_interest_rate_table, d1, d2)
